...
int tryConnect(SOCKET &socketServer, SOCKADDR_IN &client_addr, int &len)
{
    ...
    while (true)
    {
        ...
        if (header.flag == SYN && check_sum((unsigned char *)&header, sizeof(header)) == 0)
        {
            cout << "收到第一次握手请求" << endl;
            break;
        }
    ...
    if (temp_header.flag == ACK_SYN && check_sum((unsigned char *)&temp_header, sizeof(temp_header) == 0))
    {
...
int recv_message(SOCKET &socketServer, SOCKADDR_IN &client_addr, int &len, char *message)
{
    ...
    while (true)
    {
        ...
        memcpy(&header, buffer, sizeof(header));
        if (header.flag == END && check_sum((unsigned char *)&header, sizeof(header)) == 0)
        {
            cout << "文件已接收" << endl;
            break;
        }
        if (header.flag == unsigned char(0) && check_sum((unsigned char *)buffer, length - sizeof(header)))
        {
            //判断序列号是否符合
            ...
            header.SEQ = (unsigned char)seq;
            header.sum = 0;
            unsigned char temp_ = check_sum((unsigned char *)&header, sizeof(header));
            header.sum = temp_;
            memcpy(buffer, &header, sizeof(header));
            if (sendto(socketServer, buffer, sizeof(header), 0, (sockaddr *)&client_addr, len) == SOCKET_ERROR)
            {
...
int disConnect(SOCKET &socketServer, SOCKADDR_IN &client_addr, int &len)
{
    ...
    while (true)
    {
        ...
        memcpy(&header, buffer, sizeof(header));
        if (header.flag == FIN && check_sum((unsigned char *)&header, sizeof(header)) == 0)
        {
            cout << "收到第一次挥手请求" << endl;
            break;
        }
    ...
    HEADER temp_header;
    memcpy(&temp_header, buffer, sizeof(header));
    if (temp_header.flag == FIN_ACK && check_sum((unsigned char *)&temp_header, sizeof(temp_header) == 0))
    {
        cout << "收到第三次挥手请求" << endl;
    }
    else
    ...
    header.flag = FIN_ACK;
    header.sum = 0;
    temp = check_sum((unsigned char *)&header, sizeof(header));
    header.sum = temp;
    memcpy(buffer, &header, sizeof(header));
    if (sendto(socketServer, buffer, sizeof(header), 0, (sockaddr *)&client_addr, len) == SOCKET_ERROR)
    {
